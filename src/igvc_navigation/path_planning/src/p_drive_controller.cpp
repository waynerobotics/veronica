...
void set_velocity()
{
  ...
  if (abs(getDistanceError()) >= .05 && got_zero == false) //got_zero is a flag that I have made it the waypoint and stopped - time to pivot to goal pose
  {
    location_met = false;
  }
  ...
  if (waypointActive == true && angle_met == false)
  {
    ...
    cmdVel.linear.x = (abs(angularError) > .4) ? 0 : Klv * getDistanceError() / 2;
  }
  ...
  if (location_met && abs(final_desired_heading_error) < .1)
  {
