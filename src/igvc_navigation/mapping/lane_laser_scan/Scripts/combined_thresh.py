...
def dir_threshold(img, sobel_kernel=3, thresh=(0, np.pi)):
    ...
    ret, binary_output = cv2.threshold(
        absgraddir, -0.8, 0.8, cv2.THRESH_BINARY)
...
def hls_thresh2(img):
    ...
    # L (OpenCV: [0 ... 255])
    # S (OpenCV: [0 ... 255])
    # For Nnamdi's Bedroom
    white_lower = np.array(
        [np.round(0 / 2), np.round(0.6 * 255), np.round(0.0 * 255)])
    white_upper = np.array([np.round(
        360 / 2), np.round(1.00 * 255), np.round(0.4 * 255)])  # For Nnamdi's Bedroom
...
def hls_thresh_red(img):
    ...
    # S (OpenCV: [0 ... 255])
    red_lower = np.array(
        [np.round(0 / 2), np.round(0.3 * 255), np.round(0.2 * 255)])
    red_upper = np.array(
        [np.round(25 / 2), np.round(0.5 * 255), np.round(0.9 * 255)])
...
if __name__ == '__main__':
    img_file = os.path.dirname(os.path.abspath(
        __file__))+'/saves/lane_with_barrel.jpg'  # 2019-12-19-153614.jpg
    ...
    print(abs_bin.dtype, mag_bin.dtype, dir_bin.dtype,
          hls_bin.dtype, combined.dtype)
