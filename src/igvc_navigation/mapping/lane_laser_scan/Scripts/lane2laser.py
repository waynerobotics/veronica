...
class image2laser:
    ...
    def __init__(self):
        ...
        self.pubIm = rospy.Publisher('img1', Image, queue_size=10)
        self.pubIm2 = rospy.Publisher('img2', Image, queue_size=10)

        self.ls = LaserScan()
        self.ls.header.frame_id = 'laser'
    ...
    def laserScan(self, img, xc, yc, inc, amin, amax):
        xm_per_pix = 0.4/250
        ym_per_pix = 0.6/100
        ranges = np.ones([int(2*amax//inc)], dtype=float) * \
            4.0  # Initialize all to 4 meter range
    ...
    def callback(self, data):
        ...
        try:
          ...
          #  img1 = self.bridge.cv2_to_imgmsg(img, "bgr8")
         #   self.pubIm.publish(img1)
            img, _, _, _ = perspective_transform(img)
         #   img2 = self.bridge.cv2_to_imgmsg(img, "bgr8")
         #   self.pubIm2.publish(img2)
