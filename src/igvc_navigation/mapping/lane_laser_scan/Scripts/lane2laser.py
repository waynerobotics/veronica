...
count = 0
pi = np.pi
# blind angle in one quadrant = 90-70 => 140 total field of view. 70 is the actual field of view
fov = np.radians(20)
xc = (640-1)/2
...
class image2laser:
    ...
    def __init__(self):
        ...
        self.binary_lane_img = rospy.Publisher(
            "binary_lane_img", Image, queue_size=10)
        self.pub = rospy.Publisher('/lane_to_scan', LaserScan, queue_size=10)

        self.ls = LaserScan()
        self.ls.header.frame_id = 'image_laser'
        ...
        self.ls.angle_increment = 0.01
        self.ls.time_increment = 0.0001
        self.ls.scan_time = 0.046
    ...
    def laserScan(self, img, xc, yc, inc, amin, amax):
        ...
        ranges = np.ones([ranges_length], dtype=float) * \
            16.0  # Initialize all to max range
        ranges = ranges.tolist()
        # width - columns - X
        for i in range(0, img.shape[1]-1, img_scan_resolution_i):
            # height - rows - Y
            for j in range(0, img.shape[0], img_scan_resolution_j):
                if img[j][i]:
                    try:
                    ...
                    rangeIndex = min(ranges_length - 1,
                                     int((angle + amax)//inc))
    ...
    def callback(self, data):
        ...
        try:
            ...
            self.binary_lane_img.publish(
                self.bridge.cv2_to_imgmsg(img*255, "mono8"))

            self.ls.ranges = self.laserScan(
                img, xc, yc, self.ls.angle_increment, self.ls.angle_min, self.ls.angle_max)
            self.ls.header.stamp.secs = rospy.get_rostime().secs
            self.ls.header.stamp.nsecs = rospy.get_rostime().nsecs
            try:
                rospy.loginfo("Current lane2laser scan rate is {}".format(
                    1e9/(self.ls.header.stamp.nsecs - current_time)))
            except ZeroDivisionError:
                pass
