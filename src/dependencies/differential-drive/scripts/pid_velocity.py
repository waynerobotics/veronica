...
class PidVelocity():
    ...
    def calcVelocity(self):
        ...
        rospy.logdebug("-D- %s caclVelocity dt=%0.3f wheel_latest=%0.3f wheel_prev=%0.3f" %
        ...
        if (self.wheel_latest == self.wheel_prev):
            ...
            # if we got a tick right now, this would be the velocity
            ...
            if abs(cur_vel) < self.vel_threshold:
                ...
                rospy.logdebug(
                    "-D- %s below threshold cur_vel=%0.3f vel=0" % (self.nodename, cur_vel))
                self.appendVel(0)
                self.calcRollingVel()
            else:
                rospy.logdebug("-D- %s above threshold cur_vel=%0.3f" %
    ...
    def doPid(self):
        ...
        self.motor = (self.Kp * self.error) + (self.Ki *
...
#####################################END my checking here END ###########################
    ...
    def wheelCallback(self, msg):
        ...
        self.wheel_latest = 1.0 * \
            (enc + self.wheel_mult * (self.encoder_max -
